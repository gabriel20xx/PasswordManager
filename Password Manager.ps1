...
$Gui1_Btn_Add_Click= 
{
    ...
    GenerateForm2
    $Gui1_Box_List.Items.clear()
    $CSV = Import-CSV $SavePath #Variable in CSV Speichern
...
$Gui1_Btn_Delete_Click= 
{
    ...
    $Gui1_Box_List.Items.Remove($Gui1_Box_List.SelectedItem)
    ...
    $DeleteRow | export-csv $SavePath -NoTypeInformation
...
{
    $Gui2_Box_Password.Text = GenerateForm4
...
$Gui2_Btn_Add_Click=
{
    ...
    if ($Name -ne "" -and $Username -ne "" -and $Password -ne "") {  #Überprüfen, ob $Name, $Username und $Passwort leer ist
        ...
        write-host "Mindestens ein Eintrag ist leer" #Ausgabe
...
{#Correct the initial state of the form to prevent the .Net maximized form issue
    ...
    if ($EncryptedPwd -ne $null) {

        $Securepwd = $EncryptedPwd | ConvertTo-SecureString #Encrypted Passwort zu Secure Passwort konvertieren
...
function GenerateForm4 {
    ...
    $Gui4_Btn_Generate_Click=
    {
        ...
        if ($Gui4_CBox_Upper.Checked -eq $False -and $Gui4_CBox_Lower.Checked -eq $False -and $Gui4_CBox_Number.Checked -eq $False -and $Gui4_CBox_Symbol.Checked -eq $False) {
            $Error = $True
            $Gui4_Box_Output.Text = "You have to at least tick one box!"
        }

        #Überprüfen ob Minimalwert grösser ist als Maximalwert (Sollte nicht möglich sein)
        if ($Min -gt $Max) {
            $Error = $True
            $Gui4_Box_Output.Text = "The minimum value must be lower than the maximum value!"
        }

        #Überprüfen ob Anzahl-, Minimal- oder Maximalwert gleich 0 ist
        if ($Min -eq 0 -or $Max -eq 0) {
            $Error = $True
            $Gui4_Box_Output.Text = "The minimum or maximum value cant be zero!"
        }
    ...
    $System_Drawing_Size = New-Object System.Drawing.Size
    $System_Drawing_Size.Height = 23
    $System_Drawing_Size.Width = 200 #Ursprünglich 456
...
if ($TestFile -eq $False) { #Überprüfen, ob CSV Datei noch nicht vorhanden ist
    ...
    write-host "CSV Datei erstellt unter $SavePath"

} else { #Falls TestFile vorhanden ist

    write-host "CSV Datei existiert unter $SavePath"
